...
load_local_env() {
  ...
  while IFS='=' read -r env_name env_value; do
    ...
    source "${env_file}"
    env |
      grep -E '^(SERVER_PORT|DB_HOST|DB_PORT|DB_NAME|DB_USER|DB_PASSWORD|REDIS_HOST|REDIS_PORT|REDIS_PASSWORD|REDIS_DATABASE|REDIS_TIMEOUT|FLYWAY_ENABLED|DEFAULT_TENANT_ID|JWT_SECRET|ACCESS_TOKEN_MINUTES|IDLE_TIMEOUT_MINUTES|DEMO_USERNAME|DEMO_PASSWORD|BAILIAN_API_KEY|BAILIAN_TEXT_MODEL|BAILIAN_VISION_MODEL|AGENT_SERVICE_TOKEN|AGENT_SERVICE_TOKEN_TENANT_ID|AGENT_SERVICE_TOKEN_NAME|AGENT_SERVICE_TOKEN_SCOPES|AGENT_SERVICE_TOKEN_EXPIRES_AT)=' || true
  )
}
...
inherit_running_env() {
  ...
  while IFS='=' read -r env_name env_value; do
    if [[ -n "${env_name}" && -z "${!env_name:-}" ]]; then
      ...
      tr ' ' '\n' |
      grep -E '^(DB_HOST|DB_PORT|DB_NAME|DB_USER|DB_PASSWORD|REDIS_HOST|REDIS_PORT|REDIS_PASSWORD|REDIS_DATABASE|REDIS_TIMEOUT|FLYWAY_ENABLED|DEFAULT_TENANT_ID|JWT_SECRET|ACCESS_TOKEN_MINUTES|IDLE_TIMEOUT_MINUTES|DEMO_USERNAME|DEMO_PASSWORD|BAILIAN_API_KEY|BAILIAN_TEXT_MODEL|BAILIAN_VISION_MODEL|AGENT_SERVICE_TOKEN|AGENT_SERVICE_TOKEN_TENANT_ID|AGENT_SERVICE_TOKEN_NAME|AGENT_SERVICE_TOKEN_SCOPES|AGENT_SERVICE_TOKEN_EXPIRES_AT)=' || true
  )
}
...
build_start_command() {
  {
    for name in \
      ...
      BAILIAN_API_KEY BAILIAN_TEXT_MODEL BAILIAN_VISION_MODEL \
      AGENT_SERVICE_TOKEN AGENT_SERVICE_TOKEN_TENANT_ID AGENT_SERVICE_TOKEN_NAME \
      AGENT_SERVICE_TOKEN_SCOPES AGENT_SERVICE_TOKEN_EXPIRES_AT; do
      if [[ -n "${!name:-}" ]]; then
        shell_export "${name}"
      fi
    done
