...
inherit_running_env() {
  ...
  while IFS='=' read -r env_name env_value; do
    ...
    ps eww -p "${current_pid}" |
      tr ' ' '\n' |
      grep -E '^(DB_HOST|DB_PORT|DB_NAME|DB_USER|DB_PASSWORD|REDIS_HOST|REDIS_PORT|REDIS_PASSWORD|REDIS_DATABASE|REDIS_TIMEOUT|FLYWAY_ENABLED|DEFAULT_TENANT_ID|JWT_SECRET|ACCESS_TOKEN_MINUTES|IDLE_TIMEOUT_MINUTES|DEMO_USERNAME|DEMO_PASSWORD)=' || true
  )
}
