  ...
  return {
    arrangementItems: (formState.arrangementItems || [])
      ...
      .map((item) => compactObject({
        ...
        vehicleInquiryRecords: item.vehicleInquiryRecords?.length ? item.vehicleInquiryRecords
          .map((record, index) => compactObject({
            attachmentId: record.attachmentId,
            attachmentUrl: cleanProductText(record.attachmentUrl),
            availableVehicleCount: optionalNumber(record.availableVehicleCount),
            groupName: cleanProductText(record.groupName),
            includesDriverLodging: optionalBoolean(record.includesDriverLodging),
            includesDriverMeal: optionalBoolean(record.includesDriverMeal),
            includesParking: optionalBoolean(record.includesParking),
            includesToll: optionalBoolean(record.includesToll),
            inquiryMethod: record.inquiryMethod,
            inquiryPerson: cleanProductText(record.inquiryPerson),
            inquiryTime: cleanProductText(record.inquiryTime),
            quotedAmount: optionalNumber(record.quotedAmount),
            remark: cleanProductText(record.remark),
            replyPerson: cleanProductText(record.replyPerson),
            replyTime: cleanProductText(record.replyTime),
            selected: optionalBoolean(record.selected),
            sortOrder: record.sortOrder || index + 1,
            supplierId: record.supplierId,
            supplierName: cleanProductText(record.supplierName),
          })) : undefined,
        vehiclePlate: cleanProductText(item.vehiclePlate),
        vehicleQuoteSnapshot: item.vehicleQuoteSnapshot ? compactObject({
          calculatedAmount: optionalNumber(item.vehicleQuoteSnapshot.calculatedAmount),
          confirmedAmount: optionalNumber(item.vehicleQuoteSnapshot.confirmedAmount),
          endDayNo: optionalNumber(item.vehicleQuoteSnapshot.endDayNo),
          quoteRuleId: item.vehicleQuoteSnapshot.quoteRuleId,
          remark: cleanProductText(item.vehicleQuoteSnapshot.remark),
          routeSummary: cleanProductText(item.vehicleQuoteSnapshot.routeSummary),
          ruleBaseKilometers: optionalNumber(item.vehicleQuoteSnapshot.ruleBaseKilometers),
          ruleBasePrice: optionalNumber(item.vehicleQuoteSnapshot.ruleBasePrice),
          ruleCity: cleanProductText(item.vehicleQuoteSnapshot.ruleCity),
          ruleDistrict: cleanProductText(item.vehicleQuoteSnapshot.ruleDistrict),
          ruleExtraKilometerPrice: optionalNumber(item.vehicleQuoteSnapshot.ruleExtraKilometerPrice),
          ruleFloatRate: optionalNumber(item.vehicleQuoteSnapshot.ruleFloatRate),
          ruleMinimumPrice: optionalNumber(item.vehicleQuoteSnapshot.ruleMinimumPrice),
          ruleProvince: cleanProductText(item.vehicleQuoteSnapshot.ruleProvince),
          ruleVehicleType: cleanProductText(item.vehicleQuoteSnapshot.ruleVehicleType),
          scheduleEndDay: cleanProductText(item.vehicleQuoteSnapshot.scheduleEndDay),
          scheduleStartDay: cleanProductText(item.vehicleQuoteSnapshot.scheduleStartDay),
          startDayNo: optionalNumber(item.vehicleQuoteSnapshot.startDayNo),
          syncedDistanceMeters: optionalNumber(item.vehicleQuoteSnapshot.syncedDistanceMeters),
          syncedDurationSeconds: optionalNumber(item.vehicleQuoteSnapshot.syncedDurationSeconds),
        }) : undefined,
