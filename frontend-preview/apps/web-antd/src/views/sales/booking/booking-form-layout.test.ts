...
describe('sales booking order form layout', () => {
  it('keeps BookingAgent old-system sections and adds AI assisted entry', () => {
    ...
    const formalHeaderActionsSource = source.slice(source.indexOf('const formalHeaderActions'), source.indexOf('const formalHeaderBadges'));
    ...
    expect(source).not.toContain('getSalesTeamOperationDetail');
    ...
    expect(source).toContain('teamDraft.value?.routeSummary');
    ...
    expect(source).toContain('delete saveForm.receivedAmount');
    expect(source).not.toContain('receivedAmount: money(form.receivedAmount)');
    expect(source).toContain('riskApprovalRequestId: currentRiskApprovalId');
    expect(source).toContain('已有授信审批通过，可提交订单');
    expect(source).toContain('已审批通过');
    expect(source).toContain('申请授信审批');
    ...
    expect(source).toContain("import FormalTeamPageHeader from '../components/FormalTeamPageHeader.vue'");
    expect(source).toContain("import '../team-arrangement-layout.css'");
    expect(source).toContain('formalHeaderStages');
    expect(source).toContain('formalHeaderBadges');
    expect(source).toContain('formalHeaderMetrics');
    expect(source).toContain('formalHeaderActions');
    expect(source).toContain('handleFormalHeaderAction');
    expect(source).toContain('handleFormalHeaderBadge');
    expect(templateSource).toContain('<FormalTeamPageHeader');
    expect(templateSource).toContain('class="team-arrangement-card formal-team-arrangement-card booking-team-header-card"');
    expect(templateSource).toContain('kicker="收客订单 · Booking Order"');
    expect(templateSource).toContain('@note-edit="openProfileEditor(\'internal_note\')"');
    expect(templateSource).not.toContain('team-operation-header');
    expect(templateSource).not.toContain('operation-flow-row');
    expect(templateSource).not.toContain('old-system-view-actions');
    expect(templateSource).not.toContain('view-action-tile');
    expect(source).toContain('查看价格');
    expect(source).toContain('查看行程');
    expect(formalHeaderActionsSource).not.toContain("key: 'aiImport'");
    expect(templateSource).toContain('class="booking-form-toolbar"');
    expect(templateSource).toContain('@click="openAiImport"');
    ...
    expect(templateSource).toContain('@action-click="handleFormalHeaderAction"');
    expect(templateSource).toContain('@badge-click="handleFormalHeaderBadge"');
    expect(source).toContain("action.key === 'viewPrice'");
    expect(source).toContain("action.key === 'viewItinerary'");
    expect(templateSource).not.toContain('class="profile-edit-tag"');
    expect(templateSource).toContain('v-model:open="profileEditorOpen"');
    expect(templateSource).toContain(`:width="profileEditorType === 'internal_note' ? 760 : 460"`);
    expect(templateSource).toContain('class="inside-memo-textarea"');
    expect(templateSource).toContain('v-model:value="profileForm.perCapitaPitAmount"');
    expect(templateSource).toContain('v-model:value="profileForm.optionalMarkupRate"');
    expect(templateSource).toContain('v-model:value="profileForm.perCapitaShoppingAmount"');
    expect(source).toContain('perCapitaPitAmount: profileForm.perCapitaPitAmount');
    expect(source).toContain('optionalMarkupRate: profileForm.optionalMarkupRate');
    expect(source).toContain('perCapitaShoppingAmount: profileForm.perCapitaShoppingAmount');
    expect(templateSource).toContain('保存');
    expect(source).toContain('--operation-primary: #1677ff');
    expect(source).toContain('.booking-team-header-card');
    expect(source).toContain('box-shadow: 0 8px 22px rgb(15 23 42 / 5%)');
    expect(source).toContain('background: #fff !important');
    expect(source).toContain('color: var(--booking-heading)');
    ...
    expectTextOrder(source, ['收客', '排团', '发团', '结算', '完成']);
    expectTextOrder(templateSource, ["kicker=\"收客订单 · Booking Order\"", "product?.productName || '收客订单'"]);
